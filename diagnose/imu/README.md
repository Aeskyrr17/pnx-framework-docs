# IMU Demo 与 DMIMU 上层接口说明

本文说明当前工程中 DMIMU 的配置、启动、数据链路、上层订阅规则和调试方法。内容对应当前代码实现，不描述尚未实现的本地滤波或统一多 IMU 选择器。

## 当前 IMU 架构

工程中 BMI088 与 DMIMU 是两条独立服务链路：
...
`robot.json` 只描述 DMIMU 是否存在以及如何连接：
...
| 字段 | 说明 |
| --- | --- |
| `enabled` | 是否编译并允许启动 DMIMU。只有显式设为 `true` 才启用。 |
| `can_bus` | DMIMU 所在 CAN 总线，必须存在于 `board.ioc`。 |
| `can_type` | 当前 DMIMU 只支持 `classic`。 |
| `can_id` | 主控向 DMIMU 发送命令时使用的设备 ID。 |
| `master_id` | DMIMU 向主控发送数据时使用的 CAN ID。 |
...
禁用时生成 `HAS_DMIMU=0`，DMIMU 设备源码会被 CMake 排除，demo 和 AHRS 中的 DMIMU 代码也由 `#if HAS_DMIMU` 裁剪。

启用时生成的连接配置为：

```cpp
robot::imu::dmimu  // 类型为 ::imu::dmimu::transport_config
```

### `configs/params.json`

`params.json` 的 `dmimu` 分组描述运行策略和服务线程参数：
...
| 参数 | 默认值 | 规则 |
| --- | --- | --- |
| `communication_mode` | `active` | 只能为 `active` 或 `request`。 |
...
`params.dmimu` 整组或其中某一字段缺失时，生成器会逐项使用上述默认值。`params.json` 不决定 DMIMU 是否参与编译，编译开关只由 `robot.json` 决定。
...
本工程不会在初始化阶段修改 DMIMU 的通信模式、输出选择或其他持久参数。主动模式下服务线程不会发送四类数据请求，只消费设备主动上报的 CAN 帧。
...
连接配置、运行配置和服务配置在 `diagnose::imu::start()` 中组装：
...
| 配置类型 | 所属层 | 内容 |
| --- | --- | --- |
| `imu::dmimu::transport_config` | 设备层 | CAN 总线、类型和设备身份。 |
| `imu::dmimu::runtime_config` | 设备层 | 主动/请求模式和掉线阈值。 |
| `ahrs::dmimu_service_config` | 服务层 | 线程优先级、队列等待和请求周期。 |

设备层和 AHRS 服务层都不会直接读取 JSON 或包含 `robot_config.hpp`；启动/demo 层负责组装生成配置。
...
中断上下文只负责过滤和复制原始 8 字节帧，不进行浮点解析或消息发布。协议解析由设备类实现，但实际运行在 `ahrs_dmimu` 服务线程上下文中。
...
| 字段 | 单位/语义 |
| --- | --- |
| `quaternion[4]` | 顺序为 W、X、Y、Z。在线正常消息应接近单位四元数。 |
...
- 它是“最新状态”接口，不是无损采样队列；
- 推荐先调用 `msg::available()`，再调用 `msg::read()`。

## 在线、掉线与清零规则

DMIMU 只有在加速度、角速度、欧拉角和四元数四类帧全部齐全并提交完整快照后才进入在线状态。

在线判断使用最近完整快照时间，而不是最近任意 CAN 帧时间：
...
上层必须优先检查 `online`，不能把全零四元数当作有效姿态。

## Demo 调试方法

当前 demo 复用 `imu_unit_mon` 线程订阅 BMI088 与 DMIMU 两个 topic，不为 DMIMU 新增监视线程。

可在调试器 Live Watch/Expressions 中添加：
...
其中 `dmimu_demo_debug` 是面向 demo 的主要调试镜像，包含：
...
5. 不要使用 DMIMU 的 `total_yaw`，当前它固定为 0。
...
- BMI088 初始化失败会使当前 `diagnose::imu::start()` 提前返回，从而不启动 DMIMU；生产线程本身独立，但启动流程尚未完全解耦。
- BMI088 与 DMIMU 的欧拉角单位已经统一为弧度，但消息类型和有效性字段仍不统一，目前不支持业务代码无感切换数据源。
...
| 内容 | 文件 |
| --- | --- |
| DMIMU 设备接口和数据结构 | `pnx_devices/imu/dmimu/include/dmimu.hpp` |
| DMIMU 协议解析和在线审计 | `pnx_devices/imu/dmimu/src/dmimu.cpp` |
| DMIMU 服务、消息和诊断类型 | `pnx_modules/ahrs/include/ahrs.hpp` |
| DMIMU 服务线程与消息发布 | `pnx_modules/ahrs/src/ahrs.cpp` |
| 配置生成规则 | `configs/cmake/generate_config.cmake` |
| Diagnose 启动、订阅和调试镜像 | `diagnose/imu/imu_demo.cpp`、`imu_demo.hpp` |
