...
inline device_packet make_response(std::uint32_t magic, const host_packet& request,
                                   types::status status, bool ready, std::uint32_t rx_count,
                                   std::uint32_t tx_count, std::uint32_t error_count) noexcept
{
