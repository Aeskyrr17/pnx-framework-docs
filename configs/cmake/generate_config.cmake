...
foreach(hw ${PNX_IOC_FDCAN_HW})
    ...
    string(JSON board_can_capability ERROR_VARIABLE json_err GET "${board_json}" can buses ${hw_lower} capability)
    if(json_err OR board_can_capability STREQUAL "")
        message(FATAL_ERROR "board CAN profile is missing can.buses.${hw_lower}.capability")
    endif()
    string(TOLOWER "${board_can_capability}" board_can_capability)
    if(board_can_capability STREQUAL "classic")
        set(can_capability_expr "bus_capability::classic")
        set(board_can_type "classic")
    elseif(board_can_capability STREQUAL "fd_no_brs")
        set(can_capability_expr "bus_capability::fd_no_brs")
        set(board_can_type "fd")
    elseif(board_can_capability STREQUAL "fd_brs")
        set(can_capability_expr "bus_capability::fd_brs")
        set(board_can_type "fd")
    else()
        message(FATAL_ERROR "board ${hw_lower} capability must be classic, fd_no_brs, or fd_brs")
    endif()

    pnx_ioc_fdcan_frame_format("${PNX_IOC_LINES}" "${hw_lower}" ioc_can_type)
    if(NOT ioc_can_type STREQUAL board_can_type)
        message(FATAL_ERROR
            "board ${hw_lower} capability '${board_can_capability}' conflicts with IOC frame format '${ioc_can_type}'")
    endif()
    if(board_can_type STREQUAL "fd")
    ...
    if(board_tdc_enabled_cmake)
        if(NOT board_can_capability STREQUAL "fd_brs")
            message(FATAL_ERROR "board ${hw_lower} enables TDC without fd_brs capability")
        endif()
